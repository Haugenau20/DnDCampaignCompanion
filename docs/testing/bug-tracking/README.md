...
**Tracker state: 78 filed, 71 resolved, 7 open.** Filed #1414 and #1415 — both **pre-existing
defects surfaced by rewriting the tests around them**, neither introduced by the branch. Verified
against `a0e23fd`: #1414's early return is identical at `:110,121`, #1415's predicate is
byte-identical at `:78`.

Both are currently **pinned by tests that assert the defective behaviour**, each with an in-file
comment naming the root cause. That was chosen over a silently-green suite, but it sits in tension
with this repo's testing philosophy — so when either is fixed, **invert the test, do not delete it.**
They are documentation-of-defect tests, not specifications.
...
(`da724e4`), #1411 (`43f7bb0`). **Open: #1402, #1405, #1407, #1412, #1413.**
