...
| [#852](./852-chapter-progress-overwritten-not-merged.md) | 🔍 DISCOVERED | DATA | **Found 2026-07-28 while verifying #851's fix.** `updateChapterProgress` takes `Partial<ChapterProgress>` but replaces the whole per-chapter entry, so `isComplete` resets to `false` on any call that doesn't pass `true` — and `BookViewer` fires `onPageChange(page)` with no flag on every page turn. Net effect: **re-opening a finished chapter un-completes it.** Not caused by #851 (the overwrite is identical before and after), and **inert until #018 landed the same day** — that fix made progress actually persist, turning a dormant defect live | Medium | Medium | StoryContext.tsx, BookViewer.tsx |
