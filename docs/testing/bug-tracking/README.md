...
**Tracker state: 76 filed, 70 resolved, 6 open.** Filed #1400-#1413. Fixed in-session: #1400,
#1401, #1403 (`2904ed7`), #1406/#1408/#1410 and most of #1409 in the rules revision (`c749ab6`),
#1404 (`da724e4`), #1411 (`43f7bb0`). Open: #1402, #1405, #1407, #1412, #1413, and #1409's residual
two-step escalation.

⚠️ **Two things must happen outside this repo before the fixes are real:**
1. **Paste `firebase/firestore.rules.prod` into the Firebase console.** #1406, #1408, #1409 and #1410
   ...
   exposures until it is deployed.**
2. **Deploy Functions** (`firebase deploy --only functions`, region `europe-west1`) for #1403's new
   `deleteCampaign` callable. Merging without it leaves Delete failing at call time instead of
   silently no-opping. Filed #1400–#1410. Fixed in-session: #1400, #1401,
#1403 (`2904ed7`), and #1406, #1408, #1410 plus most of #1409 in the rules revision. Open: #1402,
#1404, #1405, #1407, and #1409's residual two-step escalation.

⚠️ **#1406/#1408/#1409/#1410 are fixed in `firebase/firestore.rules.prod` only — that file deploys
nothing.** They take effect when the ruleset is pasted into the Firebase console. Three of them are
live security exposures until then.
...
| [#1412](./1412-npc-directory-groups-by-location-slug-not-name.md) | 🔍 DISCOVERED | UI | **The NPC directory shows location slugs as headings** — `mines-of-moria`, `the-shire`, `rivendell` — where the Locations page shows "Mines of Moria", "The Shire", "Rivendell". `NPCDirectory:122-126` groups on `npc.location` verbatim and renders the id as the heading, with no lookup. The `'Unknown Location'` fallback works correctly, which sharpens the contrast. The same raw value feeds the filter dropdown and the `/locations?highlight=` link, so resolve names once rather than per use site. **Do not fix by prettifying the slug** — that reintroduces the `id === slugify(name)` assumption already catalogued in [#303](./303-location-parent-id-rederived-from-editable-name.md) | Medium | Medium | NPCDirectory.tsx:122-126 |
| [#1413](./1413-entity-pages-show-error-state-while-campaign-context-restores.md) | 🔍 DISCOVERED | UI | **Reloading any entity page shows a red "Please select a campaign" error for ~10 seconds** while auth and campaign context restore — then replaces itself with the full list. `hasRequiredContext` and `contextError` are both derived purely from `activeGroupId`/`activeCampaignId`, so "nothing selected" and "not restored yet" are indistinguishable and the page commits to the error state. No data is lost; the defect is that a normal successful reload is reported to the user as a failure, with an alert icon. I initially misread this as a lost session — waiting longer showed it resolving, which is itself the point: the transient state is identical to the real error. Needs a third "still resolving" state, in all four contexts at once | Medium | Medium | NPCDirectory.tsx + all four entity contexts |
...
| [#1409](./1409-member-can-escalate-to-group-admin.md) | 🟡 PARTIALLY FIXED (awaiting deploy) | VALIDATION | **A member could make themselves a group admin with one write.** The self-service profile rule placed no restriction on which fields could change, so `updateDoc({role:"admin"})` on your own `groups/{g}/users/{uid}` was **ALLOWED** — granting group deletion, member removal and token creation. Now blocked via `!…affectedKeys().hasAny(['role'])`, with all legitimate self-updates verified still working. 🟡 **The two-step path remains**: `create` must still permit `role:"admin"` because `GroupService.createGroup:101` writes the creator's own admin profile *client-side in a transaction*, so a member can leave and recreate themselves as admin. Conditioning on the group doc's `createdBy` was rejected — same transaction, so a rules `get()` may not see it, and getting that wrong breaks group creation. **Clean fix is a code change**: move group-profile creation into a Cloud Function. This bug also contaminated the probe's first run — it ran first and left the test member an admin, so every later check passed for the wrong reason | High | High | Production rules + GroupService.createGroup |
