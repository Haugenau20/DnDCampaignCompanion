...
| [#1425](./1425-anyone-could-join-any-group-without-an-invitation.md) | ✅ FIXED (awaiting deploy) | VALIDATION | **Anyone signed in could join any group without an invitation (T052).** `isGroupMember` trusts `users/{uid}.groups`, which a user could write about themselves; the registration token was checked only by the client. Measured against the live ruleset: a stranger listing a group's id in their own profile could then read and write its campaigns. Joining now runs in the `redeemInvitation` Cloud Function, which checks the token (unused, unexpired) and writes the membership in one transaction; the rules refuse any client write to `groups`. **Deploy order: functions, frontend, rules** | Critical | Critical | Production rules, InvitationService, `redeemInvitation` |
| [#1426](./1426-new-account-could-make-itself-global-admin.md) | ✅ FIXED (awaiting deploy) | VALIDATION | **Any new account could make itself a global admin.** `users/{userId}` create checked only the uid, so a fresh account's first write could carry `isAdmin: true`. Measured: allowed, and `/admin` became readable. No client may create that document now. The file gives a one-line interim hotfix that is safe to deploy on its own | Critical | Critical | Production rules |
| [#1427](./1427-members-cannot-rename-themselves.md) | ✅ FIXED (awaiting deploy) | VALIDATION | **A member could not change their name in a group.** `changeGroupUsername` deletes the old reservation, which only a global admin could do, so the whole transaction was denied. Invisible in development, where the emulator allows everything. An owner may now release their own reservation | Medium | Medium | Production rules |
