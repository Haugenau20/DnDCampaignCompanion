...
| [#303](./303-location-parent-id-rederived-from-editable-name.md) | 🔍 DISCOVERED — **proven by a failing test, not a code reading** | DATA | `LocationFormSections` re-derives `parentId` by slugifying the parent's **current display name**, but ids are fixed at creation and names are editable — so selecting a renamed location stores an id matching no location. The child appears parentless, the parent childless, and `deleteLocation`'s cascade (`getAllChildrenIds`) is handed the wrong descendant set. `strictMode` does not catch it: it validates the *name* exists, which it does. Same broken `id === slugify(name)` assumption as the collision cluster, reached from the other direction. Marker test is deliberately red in `LocationFormSections.test.tsx` — **do not fix it by editing the assertion**. Fix needs a decision: resolve the name to a real id locally (recommended, no contract change) vs. make `LocationCombobox` emit ids (cleaner, but `QuestFormSections` consumes it too) | Medium | Medium | LocationFormSections.tsx |
