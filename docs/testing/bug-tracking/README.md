...
| [#750](./750-location-create-page-initial-data-always-object.md) | 🔴 BLOCKED — characterization test | DATA | Confirmed live and the fix is written, but `LocationCreatePage.test.tsx:150` asserts the defect as the spec: `it("always passes an object (possibly with undefined fields) when no state")`, under a comment block that **cites bug #750 by number** and calls the behaviour expected. Applying the fix turns it red. Agent halted per protocol rather than editing it. **Needs the same explicit authorisation #006 and #005 got** | Low | Low | LocationCreatePage.tsx |
