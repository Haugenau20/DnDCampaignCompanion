...
| D9 | **Campaign banner** (added 2026-09-25): `Campaign.banner`, drawn into the dashboard's band exactly as D8's location picture (`BandPicture`), changeable by **any member** | Matches who may update the campaign document. Stored at `groups/{g}/campaigns/{c}/banner/`, inside the campaign's folder, so `deleteCampaign`'s prefix delete takes it; the rules and the orphan sweep name the path explicitly, as neither matches it through the entity pattern. |
