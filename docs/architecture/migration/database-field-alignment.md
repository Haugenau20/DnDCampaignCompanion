...
> (`services/firebase/…`, `src/hooks/use*Data.ts`, the old `src/components/` forms) are gone, the
> `SystemMetadataService` it assumes became `core/attribution/`, and its "no backwards compatibility,
> reset the data" option no longer applies — production holds real campaigns, so a rename needs a
> migration script.
