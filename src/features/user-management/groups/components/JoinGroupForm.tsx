// src/features/user-management/groups/components/JoinGroupForm.tsx
...
/** Props for {@link JoinGroupForm}. */
interface JoinGroupFormProps {
  /**
   * Dismisses the surface hosting the form, rendered as Cancel.
   *
   * Optional because a page has nothing to cancel back to: `/join` is a
   * destination someone followed a link to, not an interruption of what they
   * were doing. The dialog passes it and keeps its Cancel button.
   */
  onCancel?: () => void;
  /** Called once the join has succeeded. */
  onSuccess?: () => void;
}

/**
 * The join-a-group form: token, username, and the join itself.
 *
 * Lifted out of `JoinGroupDialog` verbatim so that `/join` can render it in a
 * page container while the dialog keeps rendering it inside `Dialog`. Nothing
 * about the mechanics moved: the query is still the source of the token, the
 * username check is still debounced at 500ms, and the submit still calls
 * `joinGroupWithToken`. The surface is the only thing that varies.
 */
const JoinGroupForm: React.FC<JoinGroupFormProps> = ({ onCancel, onSuccess }) => {
  ...
  // Read from the query alongside `token` and carried for the same reason the
  // dialog carried it: an invite link names the group it belongs to.
